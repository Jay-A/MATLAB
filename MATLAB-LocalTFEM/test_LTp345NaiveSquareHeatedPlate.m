...
addpath('./private/lower_triangular/');
...
[PhiRef, GradPhiRef] = PhiRefFuncs(p);
[Psi, PsiRef, GradPsiRef] = PsiFuncs(p);
...
GMass = Create_Global_Mass(p, L, Num_Global_DoFs, Num_Elements, Nodes, Elements, Areas, sgn, map);
...
for elem_cnt = 1:Num_Elements
    ...
    [Loc_RHS] = Eval_RHS(@(x,y) init_heat(x,y), Elem_Verts, Areas(elem_cnt), p, Psi);
...
%                 ,r,-1,-s ),s,-1,1 )); 
            ...
            Ke_temp(icount, jcount) = (Area/2).* (RefTri_Quad( @(r,s) ...
                GradPsiRef{jcount,1}(r,s).* (H(1,1).*GradPsiRef{icount,1}(r,s)...
                + H(1,2).*GradPsiRef{icount,2}(r,s)) + GradPsiRef{jcount,2}(r,s)...
                .*( H(2,1).*(GradPsiRef{icount,1}(r,s)) + H(2,2).*GradPsiRef{icount,2}(r,s)), p)); 
%             Ke_temp(icount, jcount) = (Area/2).* (RefTri_Quad( @(r,s) ...
